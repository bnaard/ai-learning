...
\begin{v3topic}{Denavit-Hartenberg (DH) Parameters}
    ...
    \end{equation}
\where{the four Denavit--Hartenberg parameters for joint $i$: $\theta_i$ the joint
angle, $d_i$ the link offset, $a_i$ the link length, and $\alpha_i$ the link
twist; $R$ and $T$ denote rotation and translation about the named axis.}
...
\begin{v3topic}{Forward and Inverse Kinematics}
    ...
    \end{equation}
\where{$\mathbf{q}$ is the vector of joint angles, $\mathbf{x}$ the resulting
end-effector pose, and ${}^0T_n$ the chained transform from base to tool.
Forward kinematics has one answer; the inverse generally has several or
none.}
