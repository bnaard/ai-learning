...
\begin{itemize}
  \item determine which risk tier a described system falls into under the EU AI
        Act, and what that implies;
  \item distinguish anonymisation from pseudonymisation and explain why only
        one removes data from GDPR scope;
  \item identify when a Data Protection Impact Assessment is required;
  \item explain what a right to explanation does and does not require of a
        model;
  \item state which engineering artefacts a conformity assessment expects to
        already exist.
\end{itemize}
\end{learningobjectives}

\begin{plainlanguage}
Law is not a constraint applied after the system is built --- it decides what
you may build, what data you may use, and what you must be able to show. The
useful mental model for an engineer is that regulation mostly asks for
\emph{evidence}: that you assessed risk, documented data provenance, tested for
bias, and can explain a decision to the person it affected. Almost all of that
is engineering work.
\end{plainlanguage}

\begin{engineernote}
Regulation is time-sensitive in a way the rest of this book is not. Everything
in this chapter has a date attached and may have changed since. Use it to learn
the \emph{structure} of the obligations --- risk tiers, lawful basis,
documentation duties, human oversight --- and verify current requirements against
the primary legal text and qualified counsel before any consequential decision.
...
\begin{v3topic}{IT Law: Areas and AI Relevance}
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[3]|X[3]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Areas of IT law and the conditions under which each becomes relevant to an AI system.}
\label{tab:law-areas}
...
\begin{v3topic}{Regulatory Bodies}
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Regulatory bodies and their remits.}
\label{tab:law-bodies}
\end{v3topic}
...
\begin{v3topic}{Legal Bases for Processing (Art.\ 6)}
    ...
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{The lawful bases for processing personal data. Consent is the most visible and often the weakest, because it can be withdrawn and must be freely given.}
\label{tab:law-lawful-bases}
...
\begin{v3topic}{Data Subject Rights}
    ...
    \begin{tblr}{width=\linewidth,colspec={X[2]X[4]X[4]},hlines,vlines}
        ...
        \hline
    \end{tblr}
\captionof{table}{Data subject rights and what each requires of a system. Several are engineering obligations --- deletion and portability in particular must be designed for.}
\label{tab:law-subject-rights}
\end{v3topic}
...
\begin{v3topic}{Anonymisation vs.\ Pseudonymisation}
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|X[4]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Anonymisation against pseudonymisation. Only the first removes data from GDPR scope, and the bar for it is far higher than removing identifiers.}
\label{tab:law-anon-pseudo}
...
\begin{v3topic}{Transferring Data Outside the EEA}
    ...
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Mechanisms for international data transfer. Time-sensitive: this area changes with case law, so verify before relying on any row.}
\label{tab:law-transfers}
...
\begin{v3topic}{EU AI Act: Risk-Based Classification}
    ...
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{The EU AI Act's risk tiers and the obligations attached to each. The tier follows from the use case, not from the technology.}
\label{tab:law-ai-act-tiers}
...
\begin{v3topic}{Global AI Regulatory Landscape}
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Regulatory approaches by jurisdiction. Time-sensitive: verify against current primary sources before use.}
\label{tab:law-global-landscape}
\end{v3topic}
...
\begin{v3topic}{Open-Source Licences for AI Models}
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[2]|X[6]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Licences encountered around models and datasets. Several widely-used model licences are not open-source licences at all, despite being described as such.}
\label{tab:law-licences}
...
\begin{v3topic}{eIDAS and Electronic Signatures}
    ...
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Electronic signature types and their legal standing.}
\label{tab:law-eidas}
...
\begin{v3topic}{Liability Gaps in AI Systems}
    ...
    \begin{tblr}{width=\linewidth,colspec={|X[2]|X[4]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Where existing liability frameworks fit AI poorly, and why each gap is difficult to close.}
\label{tab:law-liability-gaps}
...
\begin{cautionbox}[Anonymisation is a much higher bar than it sounds]
Under GDPR, \emph{pseudonymised} data --- identifiers replaced by keys --- is
still personal data and remains fully in scope. Only genuinely
\emph{anonymised} data, where re-identification is reasonably impossible for
anyone, falls outside. That bar is far higher than removing names.

Well-documented re-identification results show why: individuals have been
re-identified from a handful of ratings in a supposedly anonymous film dataset
by cross-referencing a public review site; from date of birth, sex, and postcode
alone; and from a few timestamped locations in mobility traces. High-dimensional
behavioural data is close to uniquely identifying by nature, so dropping the
name column changes almost nothing.

The practical consequence: assume your dataset is personal data unless a
specialist has certified otherwise, and treat model outputs as personal data too
when they can be linked back to an individual.
\end{cautionbox}

\begin{cautionbox}[What a ``right to explanation'' actually demands]
It is commonly read as requiring an interpretable model. That is not what the
obligation says. What is required is meaningful information about the
\emph{logic involved} and the significance and consequences for the person ---
which a complex model can satisfy with the right documentation, and which a
simple model can fail to satisfy if nobody wrote anything down.

What you are expected to be able to produce: which categories of data fed the
decision, what the model was optimising, the main factors affecting this
particular outcome, how the person can contest it, and who is accountable for
the review. Note that the last two are process obligations, not modelling ones
--- and they are the ones most often missing.
\end{cautionbox}

\begin{practicallab}[Assemble the compliance dossier for a system you have built]
\textbf{Goal.} Discover which required artefacts already exist and which have
never been written.

\textbf{Protocol.}
\begin{enumerate}
  \item Take any model you have built. Write one paragraph on its purpose and
        who is affected by its outputs.
  \item Classify it: does it involve personal data? Does it materially affect a
        person's access to a service, employment, credit, or safety? Which risk
        tier would that suggest?
  \item Document the data: sources, lawful basis, collection dates, retention,
        and whether consent covered this use.
  \item Document the model: architecture, training data, known limitations,
        measured performance \emph{by subgroup}, and the date of the last
        evaluation.
  \item Write the explanation a person would receive if the decision went
        against them.
  \item Define human oversight: who reviews, on what signal, with what power to
        override, and how that is logged.
  \item List every item above that did not already exist.
\end{enumerate}

\textbf{Deliverable.} The dossier, plus the gap list from step 7.
\textbf{The point:} the gap list is the real output. Nearly all of it is
engineering work that was cheaper to do during development than to reconstruct
afterwards.
\end{practicallab}

\begin{checkpoint}
A recruitment screening tool ranks candidates. Name the risk tier you would
expect it to attract, two obligations that follow, and the one measurement you
would run monthly.
\end{checkpoint}
