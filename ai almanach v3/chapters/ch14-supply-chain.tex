...
\begin{v3topic}{Exponential Smoothing (SES)}
    ...
    \end{equation}
\where{$y_t$ is the actual demand observed at time $t$, $\hat{y}_t$ the
forecast made for it, and $\alpha\in(0,1]$ the smoothing constant. Large
$\alpha$ reacts quickly to change; small $\alpha$ ignores noise.}
...
\begin{v3topic}{Classic Single-Period Newsvendor}
    ...
    \end{equation}
\where{$Q$ is the order quantity, $D$ the uncertain demand, $c_o$ the
overage cost per unsold unit and $c_u$ the underage cost per unmet unit;
$(x)^+$ means $\max(x,0)$.}
    The optimal order quantity satisfies the \textbf{critical ratio} condition:
    \begin{equation}
        F(Q^*) = \frac{c_u}{c_u + c_o} = \text{CR}
    \end{equation}
\where{$F$ is the cumulative distribution of demand, so $Q^*$ is the demand
quantile at the critical ratio CR. Read it directly: if a stockout costs three
times an unsold unit, stock to the $75\,\%$ quantile.}
...
\begin{v3topic}{Harris EOQ Formula}
    ...
    \end{equation}
\where{$\lambda$ is the demand rate per period, $K$ the fixed cost of placing
one order, and $h$ the holding cost per unit per period. The square root is why
halving order cost does not halve order size.}
