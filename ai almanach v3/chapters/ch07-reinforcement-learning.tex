...
\begin{v3topic}{The Markov Property}
        ...
        \end{equation}
\where{$s_t$ is the state at time $t$, $a_t$ the action taken there, and
$s_{t+1}$ the resulting state. The equality says the whole history on the right
adds nothing beyond the current state and action.}
...
\visualseparator
        ...
        \end{equation}
\where{$G_t$ is the return from time $t$ onward, $r_{t+k+1}$ the reward
received $k+1$ steps later, and $\gamma\in[0,1)$ the discount factor;
...
\begin{v3topic}{Value Functions}
        ...
        \end{equation}
\where{$\pi$ is the policy being followed and $s$ the state being evaluated;
the expectation runs over all trajectories the policy and environment can
produce from $s$.}
        \textbf{Action-value (Q) function} under policy $\pi$:
        \begin{equation}
            Q^{\pi}(s, a) = \mathbb{E}_{\pi}\!\left[G_t \mid S_t = s, A_t = a\right]
        \end{equation}
\where{the difference from $V^{\pi}$ is that the first action $a$ is fixed
rather than chosen by $\pi$; the policy governs everything after it.}
...
\begin{v3topic}{Bellman Equations}
        ...
        \end{equation}
\where{$\pi(a|s)$ is the probability the policy takes action $a$ in state $s$,
$P(s'|s,a)$ the environment's transition probability to next state $s'$, and
...
\begin{v3topic}{Value Iteration}
        ...
        \begin{enumerate}
            ...
            \end{equation}
\where{$k$ indexes the sweep of value iteration, not time; each sweep updates
every state using the previous sweep's values $V_k$.}
...
\begin{v3topic}{$\epsilon$-Greedy Strategy}
        ...
        \end{equation}
\where{$Q(a)$ is the running mean reward for arm $a$, $r$ the newly observed
reward, and $n$ the number of times $a$ has now been pulled --- so $1/n$ makes
this an exact incremental average.}
        \begin{itemize}
...
\begin{v3topic}{Monte Carlo Methods}
        ...
        \end{equation}
\where{$G_t$ is the full return actually observed after the first visit to
state $s$ in an episode, averaged over all episodes in which $s$ occurred.}
...
\begin{v3topic}{Temporal Difference Learning}
        ...
        \end{equation}
\where{$\alpha\in(0,1]$ is the learning rate and $\delta_t$ the TD error --- the
gap between the one-step-informed estimate $r_{t+1}+\gamma V(s_{t+1})$ and the
...
\begin{v3topic}{SARSA vs.\ Q-Learning}
        ...
        \end{equation}
\where{$s',a'$ are the next state and the action \emph{actually taken} there
by the current policy --- which is what makes SARSA on-policy.}
        ...
        \end{equation}
\where{$\max_{a'}$ takes the best available action in $s'$ regardless of what
the behaviour policy would do --- which is what makes Q-learning off-policy.}
...
\begin{v3topic}{Deep Q-Networks (DQN)}
        ...
        \end{equation}
\where{$\theta$ are the online network's parameters, $\theta^-$ the frozen
target network's, and $\mathcal{D}$ the replay buffer transitions are sampled
from. Both devices exist to stop the target moving with the estimate.}
...
\begin{v3topic}{Double Q-Learning}
        ...
        \end{equation}
\where{the inner $\arg\max$ uses the online network $\theta$ to \emph{select}
the action while the outer $Q$ uses the target network $\theta^-$ to
\emph{evaluate} it; decoupling the two removes the overestimation bias.}
...
\begin{v3topic}{The Q-Learning Update}
    ...
    \end{equation}
\where{this is the same update written as a weighted average: the old estimate
keeps weight $1-\alpha$ and the newly informed target gets weight $\alpha$.}
...
\begin{v3topic}{SARSA: On-Policy TD Control}
    ...
    \begin{equation}
        Q(s,a) \leftarrow Q(s,a) + \alpha\bigl[r + \gamma\,Q(s’,a’) - Q(s,a)\bigr]
    \end{equation}
\where{$s',a'$ are the next state and the action the policy actually takes there;
compare the Q-learning update, which substitutes $\max_{a'}Q(s',a')$.}
...
\begin{v3topic}{$\varepsilon$-Greedy Strategy}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
\where{$\varepsilon\in[0,1]$ is the exploration rate; it is usually decayed
over time, because a constant $\varepsilon$ never stops paying for exploration
it no longer needs.}
...
\begin{v3topic}{Upper Confidence Bound (UCB)}
    ...
    \end{equation}
\where{$N_t(s,a)$ is how often the pair has been tried by time $t$ and $c>0$
scales the exploration bonus, which shrinks as $N_t$ grows.}
...
\begin{v3topic}{Bayesian Inference}
    ...
    \end{equation}
\where{$\theta$ are the parameters, $\mathcal{D}$ the observed data,
$p(\theta)$ the prior, $p(\mathcal{D}|\theta)$ the likelihood, and
$p(\mathcal{D})$ the normalising evidence.}
...
\begin{v3topic}{Bayesian Networks}
    ...
    \end{equation}
\where{$\mathrm{pa}(x_i)$ denotes the parents of node $x_i$ in the graph. The
factorisation is the whole economy of the model: each variable needs a
distribution only over its parents.}
...
\begin{v3topic}{Probabilistic Modelling}
    ...
    \end{equation}
\where{$\mathbf{x}$ are observed variables, $\mathbf{z}$ latent (unobserved)
ones, and $\theta$ the model parameters.}
...
\begin{v3topic}{Conditional Independence}
    ...
    \begin{equation}
        p(X, Y | Z) = p(X|Z)\,p(Y|Z)
    \end{equation}
\where{$X$ and $Y$ are conditionally independent \emph{given} $Z$: once $Z$ is
known, $Y$ says nothing further about $X$. They may still be strongly dependent
when $Z$ is ignored.}
...
\begin{v3topic}{Backdoor Criterion}
    ...
    \end{equation}
\where{$\mathrm{do}(X)$ denotes intervening to set $X$ rather than observing
it, and $\mathcal{Z}$ is the adjustment set satisfying the backdoor criterion;
weighting by $p(\mathcal{Z}{=}z)$ reweights the strata to the population, which
is what removes the confounding.}
...
\begin{v3topic}{Front-Door Criterion}
    ...
    \end{equation}
\where{$M$ is the mediator on the causal path from $X$ to $Y$, and $x'$ ranges
over the values of $X$. The front-door formula works even when the confounder is
unobserved, provided the mediator is observed.}
