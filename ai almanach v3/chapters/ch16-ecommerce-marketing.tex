...
\begin{v3topic}{Pricing Theory Foundations}
    ...
    \end{equation}
\where{$Q$ is quantity demanded and $P$ price; $\varepsilon$ is the percentage
change in demand per percentage change in price, and is normally negative.
Demand is called elastic when $|\varepsilon|>1$.}
...
\begin{v3topic}{Bayesian Optimal Pricing}
    ...
    \end{equation}
\where{$p$ is the candidate price, $\theta$ the uncertain demand parameters,
and $Q(p,\theta)$ the resulting quantity; the expectation is taken over the
posterior for $\theta$, so the optimum accounts for what is not yet known.}
...
\begin{v3topic}{Market Basket Analysis}
    ...
    \end{equation}
\where{$A$ and $B$ are itemsets, $t$ a single transaction, and $T$ the set of
all transactions; support is simply the fraction of baskets containing both.}
    \begin{equation}
        \text{confidence}(A \Rightarrow B) = \frac{\text{support}(A \cup B)}{\text{support}(A)}
    \end{equation}
\where{confidence is the conditional probability of $B$ given $A$ --- and is
misleading on its own, because a very common $B$ produces high confidence for
any $A$.}
    \begin{equation}
        \text{lift}(A \Rightarrow B) = \frac{\text{confidence}(A \Rightarrow B)}{\text{support}(B)}
    \end{equation}
\where{lift corrects exactly that flaw by dividing by $B$'s own frequency:
$\text{lift}=1$ means independence, $>1$ genuine association, $<1$ substitution.}
...
\begin{v3topic}{User-Based Collaborative Filtering}
    ...
    \end{equation}
\where{$\hat{r}_{ui}$ is the predicted rating of user $u$ for item $i$,
$N(u)$ the set of similar users, $r_{vi}$ user $v$'s actual rating, and
$\bar{r}_u,\bar{r}_v$ their mean ratings --- subtracting them removes the fact
that some users rate generously and others harshly.}
    \textbf{Cosine similarity} between user rating vectors:
    \begin{equation}
        \text{sim}(u,v) = \frac{\mathbf{r}_u \cdot \mathbf{r}_v}{\|\mathbf{r}_u\|\,\|\mathbf{r}_v\|}
    \end{equation}
\where{$\mathbf{r}_u$ is user $u$'s rating vector; this is the cosine
similarity of \cref{ch:mathematical-foundations} applied to rating rows.}
...
\begin{v3topic}{Item-Based Collaborative Filtering}
    ...
    \end{equation}
\where{$N(i)$ is the set of items similar to $i$ and $r_{uj}$ the same user's
rating of a similar item --- the item-based mirror of the user-based formula
above, and usually the more stable of the two because items change less than
people.}
...
\begin{v3topic}{Matrix Factorisation}
    ...
    \end{equation}
\where{$\mathcal{K}$ is the set of \emph{observed} ratings only,
$\mathbf{p}_u$ and $\mathbf{q}_i$ the latent factor vectors for user and item,
and $\lambda$ the regularisation strength. Summing only over $\mathcal{K}$ is
essential --- the unobserved entries are unknown, not zero.}
...
\begin{v3topic}{Bayesian Personalised Ranking (BPR)}
    ...
    \end{equation}
\where{$i^+$ is an item the user interacted with, $j^-$ one they did not,
$\sigma$ the logistic function, and $\Theta$ all model parameters. The loss
ranks pairs rather than predicting ratings, which suits implicit feedback where
no explicit score exists.}
...
\begin{v3topic}{Factorisation Machines}
    ...
    \end{equation}
\where{$w_0$ is the global bias, $w_i$ the weight on feature $i$, and
$\mathbf{v}_i$ a latent vector per feature; the inner product
$\langle\mathbf{v}_i,\mathbf{v}_j\rangle$ models the interaction between two
features without needing a separate parameter for every pair.}
...
\begin{v3topic}{Wide \& Deep / Neural CF}
    ...
    \end{equation}
\where{$[\mathbf{p}_u \| \mathbf{q}_i]$ is the concatenation of the user and
item embeddings, the MLP learns their interaction, and $\mathbf{h}$ is the
output layer; $\sigma$ maps the score to a probability.}
