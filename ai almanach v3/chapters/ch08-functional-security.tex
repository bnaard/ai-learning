...
\chapterlead{What evidence justifies trust within a stated operating boundary?}{Scope}{Threats}{Controls}{Assure}{trustworthy-ai}
...
\end{tblr}
\captionof{table}{Trustworthiness properties, each paired with the engineering question it answers and the evidence that would settle it.}
\label{tab:sec-trust-properties}
\end{center}
...
\end{tblr}
\captionof{table}{Machine-learning failure families by origin. Note how few are model failures: most originate in specification or data.}
\label{tab:sec-failure-families}
\end{center}
...
\end{align}
\where{$\tau$ is the confidence threshold, $A_\tau$ the set of inputs the model
accepts rather than abstaining on, $n$ the total number of inputs, and $\ell$
the loss. Raising $\tau$ lowers coverage and should lower risk --- if it does
not, the confidence score is not informative.}
...
\end{tblr}
\captionof{table}{Adversarial attacks by lifecycle stage, with a corresponding control. No single defence covers the whole table, which is why the last column is defence in depth.}
\label{tab:sec-adversarial-controls}
\end{center}
...
\end{equation}
\where{$M$ is the randomised mechanism, $D$ and $D'$ two datasets differing in one
individual, and $S$ any set of outputs. $\varepsilon$ is the privacy budget
(smaller is stronger) and $\delta$ the probability the guarantee fails
altogether.}
...
\begin{v3topic}{Treat Tool Output as Untrusted}
...
\end{v3topic}
\begin{v3topic}{Least-Privilege Tool Use}
...
\end{v3topic}
\begin{v3topic}{Trajectory-Level Assurance}
...
\chapterreview{Scope}{Threats}{Controls}{Assure}
