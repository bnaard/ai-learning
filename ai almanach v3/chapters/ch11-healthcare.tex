...
\begin{itemize}
  \item compute the positive predictive value of a screening test at a
        realistic prevalence and explain the result to a non-specialist;
  \item distinguish retrospective accuracy from clinical utility and name the
        study design that establishes the second;
  \item identify the shortcut a medical imaging model is most likely to learn;
  \item explain why performance on one hospital's data says little about
        another's;
  \item state what a regulator requires of a clinical decision support tool
        beyond a validation score.
\end{itemize}
\end{learningobjectives}

\begin{plainlanguage}
Medicine is the domain where the arithmetic of \cref{ch:mathematical-foundations}
stops being academic. A test that is right $99\,\%$ of the time can still be
wrong more often than right when the disease is rare --- and that is not a
paradox, it is the base rate. Almost every disappointment in clinical AI traces
back either to that arithmetic or to a dataset that encoded the hospital rather
than the disease.
\end{plainlanguage}

\begin{engineernote}
The gap that matters here is between \emph{accuracy} and \emph{utility}. A model
can be more accurate than a clinician and still change no outcome --- because it
detects something at a stage where treatment does not differ, or because it
fires too late in the workflow to act on, or because clinicians do not trust it
and override it. Retrospective accuracy is necessary and nowhere near
sufficient. The evidence that counts comes from a prospective study measuring
patient outcomes, not model scores.
\end{engineernote}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,3cm]|X[l]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Healthcare stakeholders and where AI can help each. Note that the stakeholder who benefits is often not the one who must change behaviour.}
\label{tab:health-stakeholders}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2.5cm]|X[l]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{The drug discovery pipeline and where machine learning applies. Compression at an early stage is worth little if the later, more expensive stages are unchanged.}
\label{tab:health-drug-pipeline}
...
\begin{v3topic}{Modality Comparison}
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,1.8cm]|X[l]|X[l]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Imaging modalities compared by signal, contrast, and radiation dose --- the three properties that decide which is clinically appropriate.}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2cm]|X[l]|X[l]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Landmark medical imaging results. Read the last column with the base-rate arithmetic of this chapter in mind before translating it into clinical value.}
\label{tab:health-imaging-results}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2.2cm]|X[l]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Regulatory frameworks for clinical software and their central requirement. Time-sensitive: verify against current primary sources.}
\label{tab:health-regulatory}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2cm]|X[l]|X[l]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Clinical terminologies and coding systems. Mapping between them is lossy, which is a recurring source of error in multi-site studies.}
\label{tab:health-terminologies}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2.5cm]|X[l]|}}
        \hline
        ...
        \hline
    \end{tblr}
\captionof{table}{Components of a surgical robot system. These are teleoperated instruments, not autonomous agents --- the surgeon remains in the loop throughout.}
\label{tab:health-surgical-robot}
...
\begin{v3topic}{Privacy-Preserving ML Techniques}
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,2.5cm]|X[l]|X[l]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Privacy-preserving techniques and what each costs. There is no option in this table that is free in both accuracy and complexity.}
\label{tab:health-privacy-ml}
...
\visualseparator
    \begin{tblr}{width=\linewidth,colspec={|Q[l,m,3cm]|X[l]|X[l]|}}
        ...
        \hline
    \end{tblr}
\captionof{table}{Fairness metrics and their clinical meaning. The metrics are mutually incompatible in general, so choosing one is a clinical and ethical decision, not a technical one.}
\label{tab:health-fairness}
...
\begin{workedexample}[A screening test at a realistic prevalence]
A deep-learning screening tool reports $95\,\%$ sensitivity and $95\,\%$
specificity for a condition with prevalence $0.5\,\%$. Screen $100{,}000$
people.

\smallskip
\begin{tblr}{
  width=0.86\linewidth,
  colspec={Q[l,0.24\linewidth] Q[c] Q[c] Q[c]},
  row{1}={font=\bfseries,bg=AlmanachMist},
  hlines,vlines}
& Has condition & Does not & Total \\
Test positive & $475$ & $4{,}975$ & $5{,}450$ \\
Test negative & $25$ & $94{,}525$ & $94{,}550$ \\
Total & $500$ & $99{,}500$ & $100{,}000$ \\
\end{tblr}
\smallskip

\[
  \text{PPV} = \frac{475}{5{,}450} = \mathbf{8.7\,\%},
  \qquad
  \text{NPV} = \frac{94{,}525}{94{,}550} = 99.97\,\% .
\]

\emph{Read it.} Of the people told they may have the condition, fewer than one
in eleven does. Nearly $5{,}000$ healthy people receive a positive result and
proceed to confirmatory testing --- with its own cost, waiting time, risk, and
anxiety. Meanwhile $25$ genuine cases are missed.

\emph{This is not a bad model.} $95/95$ is excellent. The number that dominates
is the prevalence, and no improvement in the model changes that arithmetic
much: pushing specificity to $99\,\%$ raises PPV to about $32\,\%$, which is
better and still means two of every three positives are false. This is why
screening programmes target enriched populations rather than everyone, and why
sensitivity and specificity must never be quoted to patients as if they were
predictive values.
\end{workedexample}

\begin{cautionbox}[Medical imaging models learn the hospital]
Documented shortcuts, all of which produced excellent held-out accuracy:
a model detecting pneumonia from the scanner model rather than the lung, because
the sicker hospital used a different machine; a model detecting malignancy from
the presence of a surgical ruler, placed in photographs only when a
dermatologist already suspected cancer; a model reading a chest drain --- which
is placed \emph{because} of the condition being predicted --- as evidence of the
condition; a model using image laterality markers or patient-position text
burned into the corner of the frame.

Each is invisible in a random split, because the shortcut is present on both
sides. The defence is external validation: data from a different hospital,
scanner, and population, collected under a different protocol. A model that has
not been validated externally has not been validated.
\end{cautionbox}

\begin{practicallab}[Turn an accuracy claim into a utility claim]
\textbf{Goal.} Take a published medical AI result and work out what it would
mean in a clinic.

\textbf{Protocol.}
\begin{enumerate}
  \item Find a paper reporting sensitivity and specificity for a diagnostic
        model. Record both, and the study population.
  \item Look up the prevalence in the population where it would actually be
        deployed --- not the study's enriched cohort.
  \item Build the $2\times2$ table for $100{,}000$ people. Compute PPV and NPV.
  \item Compute the downstream volume: how many confirmatory tests per year,
        at what cost and waiting time.
  \item Ask what changes for a patient. If the model fires, what happens next
        that would not otherwise have happened?
  \item Check the paper for external validation. Note the scanner, site, and
        demographic range of the training data.
\end{enumerate}

\textbf{Deliverable.} The $2\times2$ table, the PPV, the annual confirmatory
test volume, and one sentence on whether the model changes any decision.
\textbf{The point:} step 5 is the question a clinician will ask first and the
one papers most often leave unanswered.
\end{practicallab}

\begin{checkpoint}
A vendor reports AUC $0.94$ for sepsis prediction, validated on data from three
hospitals. List three questions you would ask before a pilot, one of which must
concern how the alert fits into an existing clinical workflow.
\end{checkpoint}
