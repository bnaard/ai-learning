...
\chapterlead{What evidence shows that a learned system remains useful after release?}{Contract}{Test}{Release}{Monitor}{evaluation-mlops}
...
\begin{v3topic}{Decision and action}
...
\end{v3topic}
\begin{v3topic}{Unit and population}
...
\end{align}
\where{$TP,FP,TN,FN$ are the four confusion-matrix counts: true and false
positives, true and false negatives. Precision divides by what was predicted
positive, recall by what actually was positive.}
...
\begin{align}
    ...
    \label{eq:mse-rmse}
\end{align}
\where{$e_i = y_i - \hat y_i$ is the residual for observation $i$ and $n$ the
number of observations. MSE squares the residuals, so a single large error
dominates it in a way it cannot dominate MAE.}
...
\begin{equation}
    ...
    \label{eq:r-squared}
\end{equation}
\where{$y_i$ is the actual value, $\hat y_i$ the prediction, and $\bar y$ the mean
of the actuals; the denominator is the error a constant-mean model would make,
so $R^2$ is the fraction of that error removed.}
...
\end{equation}
\where{$p_i\in[0,1]$ is the predicted probability and $y_i\in\{0,1\}$ the actual
outcome; lower is better. Unlike accuracy it rewards being well calibrated, not
just being on the right side of the threshold.}
...
\begin{v3topic}{Quality Is a Vector}
...
\end{v3topic}
\begin{v3topic}{Judges Need Evaluation Too}
...
\end{v3topic}
\begin{v3topic}{Sequential Release Evidence}
...
\chapterreview{Contract}{Test}{Release}{Monitor}
