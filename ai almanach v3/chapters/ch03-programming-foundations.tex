...
\begin{workedexample}[LEGB resolution, and the error it causes]
\emph{How the rule works.} Python looks for a name in four scopes, innermost
...
The name \texttt{x} is not Local to \texttt{inner}, so Python looks Enclosing
and finds \texttt{outer}'s \texttt{x}. The Global \texttt{x} is never reached.

\emph{Now the error every Python programmer meets once.}
...
This raises \texttt{UnboundLocalError: cannot access local variable 'count'},
which looks absurd --- \texttt{count} is clearly defined on the line above the
function.

\emph{Why.} Python decides scope at \emph{compile} time, per function, from
whether the name is ever assigned in it. Because \texttt{count} is assigned
somewhere in \texttt{bump}, it is Local \emph{throughout} \texttt{bump} ---
including on the right-hand side, where it has not been given a value yet. The
Global \texttt{count} is shadowed by a local that does not exist yet.

\emph{The fixes,} in order of preference: pass the value in and return the new
one; use \texttt{nonlocal} for an enclosing scope; use \texttt{global} only as
a last resort. The rule to remember is that \emph{assignment anywhere in a
function makes the name local everywhere in that function}.
\end{workedexample}
\end{v3topic}
